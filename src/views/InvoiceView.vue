...
export default {
  ...
  methods: {
    ...
    getCurrentInvoice() {
      ...
      this.currentInvoice = this.currentInvoiceArray[0]; // [0] because filter returns arr
    },
    toggleEditInvoice() {
      this.TOGGLE_EDIT_INVOICE();
      this.TOGGLE_INVOICE(); // show edit modal
    },

    async deleteInvoice(docId) {
      await this.DELETE_INVOICE(docId);
      this.$router.push({ name: "Home" }); // redirect back to home
    },
  ...
  watch: {
    editInvoice() {
      //    watch any changes on this.editInvoice ==> when the user editing and not editing
      if (!this.editInvoice) {
